...
def extract_weconnectu_excel(uploaded_file) -> list:
    """Read WeConnectU Actual vs Budget (YTD Actual + TOTAL Budget)."""
    ...
    for sheet in xl.sheet_names:
        ...
        for i in range(min(15, len(df))):
            ...
            if "actual" in vals and "budget" in vals:
                header_row = i
                break
        if header_row is None:
            continue
        prev = [str(v).strip().lower() for v in df.iloc[header_row - 1].tolist()] if header_row > 0 else [""] * df.shape[1]
        ...
        ytd_budget = next((i for i, t in enumerate(merged) if "ytd" in t and "budget" in t), None)
        ...
        for r in range(header_row + 1, len(df)):
            ...
            if gl.endswith("/000") or re.search(r"do not use|duplicated", desc, re.I):
                continue
            if gl.startswith("3000") or gl.startswith("3300"):
                continue
            actual = pd.to_numeric(df.iloc[r, ytd_actual], errors="coerce")
            actual = 0.0 if pd.isna(actual) else float(actual)
            if abs(actual) < 0.01:
                actual = 0.0
            bud_col = total_budget if total_budget is not None else ytd_budget
            budget = None
            if bud_col is not None:
                b = pd.to_numeric(df.iloc[r, bud_col], errors="coerce")
                if pd.notna(b) and abs(float(b)) >= 0.5:
                    budget = abs(float(b))
            if abs(actual) < 0.5 and budget is None:
                continue
            rows_out.append({"desc": desc, "gl": gl, "actual": abs(actual), "budget": budget})
    return rows_out


def apply_weconnectu_rows(sections: dict, wcu_rows: list):
    used = set()
    insurance = 0.0

    def score(item, row):
        if item.get("gl") and row["gl"] and _norm(item["gl"]) == _norm(row["gl"]):
            return 1.0
        a, b = _norm(item.get("desc", "")), _norm(row["desc"])
        if a == b:
            return 0.95
        if a in b or b in a:
            return 0.85
    ...
    leftover = []
    for row in wcu_rows:
        if re.search(r"insurance\s*claim", row["desc"], re.I):
        ...
        for key, items in sections.items():
            for idx, item in enumerate(items):
                ...
                s = score(item, row)
                if s > best_s:
                    best_s, best = s, (key, idx)
        if best and best_s >= 0.8:
            key, idx = best
            used.add(f"{key}:{idx}")
            item = sections[key][idx]
            item["actual"] = row["actual"]
            if row["gl"]:
                item["gl"] = row["gl"]
            if row.get("budget") is not None:
                item["budgeted"] = row["budget"]
                if row["actual"]:
                    item["pct"] = (row["budget"] - row["actual"]) / row["actual"] * 100
            else:
                item["budgeted"] = row["actual"] * (1 + float(item.get("pct") or 0) / 100)
        else:
            leftover.append(row)

    def section_for(gl, desc):
        p = (gl or "").split("/")[0]
        d = desc.lower()
        if p == "2100":
            return "municipal"
        ...
        if p in ("4700", "4800"):
            return "tax"
        if p == "1000":
            if "csos levy" in d and "collection" not in d:
                return "levy_income"
            if "reserve" in d or re.match(r"^(levies|levy)\b", d):
                return "levy_income"
            if "recover" in d and re.search(r"water|electric|sewer|refuse", d):
    ...
    added = 0
    for row in leftover:
        key = section_for(row["gl"], row["desc"])
        sections[key].append({
            "desc": row["desc"],
            "gl": row["gl"],
            "actual": row["actual"],
            "pct": ((row["budget"] - row["actual"]) / row["actual"] * 100) if row.get("budget") and row["actual"] else 0.0,
            "budgeted": row.get("budget") if row.get("budget") is not None else row["actual"],
            "note": "Added from WeConnectU — unique to this complex",
        })
        added += 1
    return sections, insurance, added
...
def main():
    ...
    if logo_path.exists():
        ...
        with col_title:
            ...
            st.caption("Upload WeConnectU Excel (preferred) or a PDF → review → download Excel")
    else:
        st.title("Domus Property Management Budget")
        st.caption("Upload WeConnectU Excel (preferred) or a PDF → review → download Excel")
    ...
    with st.sidebar:
        ...
        st.header("2. Load last year’s figures")
        excel_up = st.file_uploader("WeConnectU Excel (Budget vs Actuals)", type=["xlsx", "xls", "xlsm", "csv"], key="wcu_excel")
        if excel_up and st.button("Load WeConnectU Excel", type="primary"):
            with st.spinner("Reading Excel…"):
                try:
                    wcu = extract_weconnectu_excel(excel_up)
                    if not wcu:
                        st.error("No GL lines found. Export Options → Budget and Actuals from WeConnectU.")
                    else:
                        sections, ins, added = apply_weconnectu_rows(default_sections(), wcu)
                        st.session_state.sections = sections
                        if ins:
                            st.session_state.insurance_recoveries = ins
                        for k in list(st.session_state.keys()):
                            if str(k).startswith("df_"):
                                del st.session_state[k]
                        st.success(f"Loaded {len(wcu)} lines. Extra complex-specific lines added: {added}.")
                        st.rerun()
                except Exception as e:
                    st.error(f"Could not read Excel: {e}")

        uploaded = st.file_uploader("Financial statement PDF (backup only)", type=["pdf"])
