...
Upload WeConnectU Actual vs Budget Excel (or PDF) → extract actuals → review → download Excel
"""
...
# ---------------------------------------------------------------------------
# WECONNECTU EXCEL EXTRACTION (primary method – reliable across complexes)
# ---------------------------------------------------------------------------

# Map (GL code prefix / keyword patterns) → internal actuals keys.
# Order matters for overlapping patterns: more specific first.
EXCEL_RULES = [
    # --- Income ---
    (r"^1000/001", r"lev(y|ies)", "ordinary_levies"),
    (r"^1000/018|^1000/027", r"reserve", "reserve_levies"),
    (r"^1000/017|^1000/018|^1000/019|^1000/029|^1000/049", r"csos", "csos_income"),
    (r"^1000/003", r"arrears|interest.*arrear", "interest_arrears"),
    (r"^1000/004", r"bank|marketlink|interest.*received|investment", "investment_income"),
    (r"^1000/010", r"penalty", "penalty_income"),
    (r"^1000/011", r"water\s*recover", "water_recovered"),
    (r"^1000/012", r"sewer(age)?\s*recover", "sewerage_recovered"),
    (r"^1000/013", r"electric(ity)?\s*recover", "electricity_recovered"),
    (r"^1000/014|^1000/021", r"refuse\s*recover", "refuse_recovered"),
    (r"^1000/008|^1000/018", r"insurance\s*(claim|additional)|additional\s*insurance", "ins_claims_received"),
    (r"^1000/005", r"rental", "other_income_rental"),
    (r"^1000/020|^1000/023|^1000/051", r"legal\s*fees?\s*recover", "legal_fees_recovered"),
    # --- Municipal expenses (gross) ---
    (r"^2100/001", r"^water$|water\s*(?!recover)", "water_gross"),
    (r"^2100/002", r"refuse", "refuse_gross"),
    (r"^2100/003", r"sewer", "sewerage_gross"),
    (r"^2100/004", r"electric", "electricity_gross"),
    (r"^2100/005", r"rates|municipality|property\s*tax", "rates_gross"),
    # --- Admin / Expenditure ---
    (r"^2000/001", r"bank\s*charge", "bank_charges"),
    (r"^2000/002", r"management\s*fee", "management_fee"),
    (r"^2000/003", r"cleaning", "cleaning"),
    (r"^2000/007", r"legal", "legal"),
    (r"^2000/011", r"audit", "audit_fees"),
    (r"^2000/016|^2000/029", r"insurance", "insurance"),
    (r"^2000/015|^2000/037", r"security|guarding", "security"),
    (r"^2000/022|^2000/031|^2000/034", r"csos", "csos_expense"),
    (r"^2000/026|^2000/010|^2000/060|^2000/021", r"accounting", "accounting_fees"),
    (r"^2000/023|^2000/027|^2000/028", r"meeting|venue|refreshment", "venue_hire"),
    (r"^2000/008|^2000/009|^2000/033", r"telephone|telkom|internet|communication|airtime", "telephone"),
    (r"^2000/005", r"office|general\s*office|sundry", "office_expenses"),
    (r"^2000/004", r"computer|it\s*expense", "computer_expenses"),
    (r"^2000/018", r"health\s*&?\s*safety", "health_safety"),
    (r"^2000/023", r"print|stationer", "printing"),
    # --- R&M ---
    (r"^2200/001", r"fire", "fire_equipment"),
    (r"^2200/002", r"general\s*building|building", "general_building"),
    (r"^2200/003", r"plumb|sewerage\s*&?\s*plumb", "plumbing"),
    (r"^2200/004", r"gate|intercom", "gate_intercom"),
    (r"^2200/005", r"electric", "electrical"),
    (r"^2200/006", r"electric\s*fence|fence", "electric_fence"),
    (r"^2200/009", r"garden", "garden"),
    (r"^2200/012", r"pool|swimming", "pool"),
    (r"^2200/017|^2200/027", r"insurance\s*claim|maintenance\s*.*insurance", "ins_claims_rm"),
    (r"^2200/000", r"^maintenance$|^maintenance\s*&?\s*repairs$", "repairs_total"),
    # --- Personnel ---
    (r"^4000/003", r"casual|relief", "casual_wages"),
    (r"^4000/001|^4000/018", r"salar|staff\s*wage|^wages$|wages\s*$", "salaries"),
    (r"^4000/003", r"staff\s*wage|wages", "salaries"),
    (r"^4000/004|^4000/005", r"paye|uif", "paye_uif"),
    (r"^4000/006", r"travel", "travel"),
    (r"^4000/007", r"bonus|overtime", "bonuses"),
    (r"^4000/009", r"wca|coida", "wca"),
    (r"^4000/014", r"pension|provident", "pension"),
    (r"^4000/015|^4000/011", r"protective|clothing|welfare", "staff_welfare"),
    # --- Tax ---
    (r"^4700/|^4800/", r"tax|taxation", "taxation"),
    # --- Special projects ---
    (r"^2300/", r".", "special_projects_total"),
]
...
def extract_actuals_from_excel(uploaded_file):
    """
    Parse a WeConnectU 'Actual vs Budget' Excel export.
    Returns (actuals_dict, complex_name, period_str).
    """
    ...
    for r in range(1, 5):
        for c in range(1, 5):
            ...
            if re.search(r"\d{4}-\d{2}-\d{2}", s):
                period = s
            if "body corporate" in s.lower() or "home owners" in s.lower() or "hoa" in s.lower() or "association" in s.lower():
                complex_name = s
            elif r == 2 and c == 1 and len(s) > 3 and not s.lower().startswith("actual"):
                complex_name = s
    ...
    for r in range(1, 10):
        for c in range(1, min(30, ws.max_column + 1)):
            v = ws.cell(r, c).value
            if v is not None and str(v).strip().upper() == "YTD":
                below = ws.cell(r + 1, c).value
                if below is not None and "actual" in str(below).lower():
                    ytd_col = c
                    header_row = r + 1
                    break
                if "actual" in str(v).lower():
                    ytd_col = c
                    header_row = r
                    break
        if ytd_col:
    ...
    for r in range((header_row or 5) + 1, ws.max_row + 1):
        label = ws.cell(r, 1).value
        if not label or not isinstance(label, str):
            continue
        label = label.strip()
        if re.match(r"^(TOTAL|SURPLUS)", label, re.I):
            continue

        amount = _safe_float(ws.cell(r, ytd_col).value)
        if abs(amount) < 1:
            continue
        ...
        # Skip parent section totals like 2000/000, 2200/000 (double-count detail lines)
        if re.match(r"^\d{4}/000$", code):
            continue

        matched = False
        for code_pat, desc_pat, key in EXCEL_RULES:
            code_ok = re.search(code_pat, code, re.I) if code else False
            desc_ok = re.search(desc_pat, desc, re.I) or re.search(desc_pat, label, re.I)
            if code_ok and desc_ok:
                if key in actuals:
                    actuals[key] = actuals[key] + amount
                else:
                    actuals[key] = amount
                matched = True
                break
            if code_ok and key in (
                "ordinary_levies", "water_gross", "electricity_gross", "sewerage_gross",
                "refuse_gross", "rates_gross", "bank_charges", "management_fee",
                "insurance", "security", "accounting_fees", "audit_fees",
                "salaries", "casual_wages", "bonuses", "paye_uif", "taxation",
                "fire_equipment", "plumbing", "electrical", "gate_intercom",
                "garden", "pool", "general_building", "electric_fence",
            ):
        ...
        if not matched and amount > 100:
            unmatched.append((label, amount))

    actuals["_unmatched"] = unmatched
    actuals["_complex_name"] = complex_name
    actuals["_period"] = period
    return actuals, complex_name, period
...
def apply_actuals(sections: dict, actuals: dict) -> dict:
    ...
    for item in sections["levy_income"]:
        if "Ordinary" in item["desc"] and actuals.get("ordinary_levies"):
            item["actual"] = abs(float(actuals["ordinary_levies"]))
        if "CSOS" in item["desc"] and actuals.get("csos_income"):
            item["actual"] = abs(float(actuals["csos_income"]))
        if "Reserve" in item["desc"] and actuals.get("reserve_levies"):
            item["actual"] = abs(float(actuals["reserve_levies"]))

    for item in sections["other_income"]:
        if "Interest on Arrears" in item["desc"] and actuals.get("interest_arrears"):
            item["actual"] = abs(float(actuals["interest_arrears"]))
        if "Investment" in item["desc"] and actuals.get("investment_income"):
            item["actual"] = abs(float(actuals["investment_income"]))
        if "Penalty" in item["desc"] and actuals.get("penalty_income"):
            item["actual"] = abs(float(actuals["penalty_income"]))
        if "Other Income 1" in item["desc"] and actuals.get("other_income_rental"):
            item["actual"] = abs(float(actuals["other_income_rental"]))
            item["desc"] = "Rental / Other Income"
        if "Other Income 2" in item["desc"] and actuals.get("legal_fees_recovered"):
            item["actual"] = abs(float(actuals["legal_fees_recovered"]))
            item["desc"] = "Legal Fees Recovered"
    ...
    for item in sections["muni_recoveries"]:
        if "Electricity" in item["desc"] and actuals.get("electricity_recovered"):
            item["actual"] = abs(float(actuals["electricity_recovered"]))
        if "Water" in item["desc"] and actuals.get("water_recovered"):
            item["actual"] = abs(float(actuals["water_recovered"]))
        if "Sewerage" in item["desc"] and actuals.get("sewerage_recovered"):
            item["actual"] = abs(float(actuals["sewerage_recovered"]))
        if "Refuse" in item["desc"] and actuals.get("refuse_recovered"):
            item["actual"] = abs(float(actuals["refuse_recovered"]))

    # Municipal expenses (gross)
    for item in sections["municipal"]:
        if item["desc"] == "Water" and actuals.get("water_gross"):
            item["actual"] = abs(float(actuals["water_gross"]))
        if "Electricity" in item["desc"] and actuals.get("electricity_gross"):
            item["actual"] = abs(float(actuals["electricity_gross"]))
        if "Sewerage" in item["desc"] and actuals.get("sewerage_gross"):
            item["actual"] = abs(float(actuals["sewerage_gross"]))
        if "Refuse" in item["desc"] and actuals.get("refuse_gross"):
            item["actual"] = abs(float(actuals["refuse_gross"]))
        if "Rates" in item["desc"] and actuals.get("rates_gross"):
            item["actual"] = abs(float(actuals["rates_gross"]))
    ...
    op_map = {
        ...
        "Insurance": "insurance",
        "CSOS Levies (Expense)": "csos_expense",
        ...
        "Garden Service (contract)": "garden_service",
        "Cleaning & Materials": "cleaning",
        "Health & Safety": "health_safety",
        "Computer / IT Expenses": "computer_expenses",
        "Printing & Stationery": "printing",
    }
    for item in sections["expenditure"]:
        k = op_map.get(item["desc"])
        if k and actuals.get(k):
            item["actual"] = abs(float(actuals[k]))
        if item["desc"] == "CSOS Levies (Expense)" and not item.get("actual") and actuals.get("csos_income"):
            item["actual"] = abs(float(actuals["csos_income"]))

    # Personnel
    for item in sections["personnel"]:
        if item["desc"].startswith("Salaries") and actuals.get("salaries"):
            item["actual"] = abs(float(actuals["salaries"]))
        if "Casual" in item["desc"] and actuals.get("casual_wages"):
            item["actual"] = abs(float(actuals["casual_wages"]))
        if "Bonuses" in item["desc"] and actuals.get("bonuses"):
            item["actual"] = abs(float(actuals["bonuses"]))
        if "PAYE" in item["desc"] and actuals.get("paye_uif"):
            item["actual"] = abs(float(actuals["paye_uif"]))
        if "Travel" in item["desc"] and actuals.get("travel"):
            item["actual"] = abs(float(actuals["travel"]))
        if "WCA" in item["desc"] and actuals.get("wca"):
            item["actual"] = abs(float(actuals["wca"]))
        if "Pension" in item["desc"] and actuals.get("pension"):
            item["actual"] = abs(float(actuals["pension"]))
        if "Staff Welfare" in item["desc"] and actuals.get("staff_welfare"):
            item["actual"] = abs(float(actuals["staff_welfare"]))
    ...
    rm_map = {
        ...
        "General / Buildings Maintenance": "general_building",
        "Pool": "pool",
    ...
    if actuals.get("repairs_total"):
        specific_sum = sum(
            abs(float(actuals.get(k) or 0))
            for k in ("electrical", "fire_equipment", "plumbing", "gate_intercom",
                      "garden", "general_building", "pool", "electric_fence")
        )
        for item in sections["rm"]:
            if "General" in item["desc"] and not item.get("actual"):
                residual = abs(float(actuals["repairs_total"])) - specific_sum
                if residual > 100:
                    item["actual"] = residual
                elif specific_sum < 1:
                    item["actual"] = abs(float(actuals["repairs_total"]))
                break

    # Tax
    for item in sections["tax"]:
        if actuals.get("taxation"):
            item["actual"] = abs(float(actuals["taxation"]))

    # Special projects
    if actuals.get("special_projects_total"):
        for item in sections["special"]:
            if not item.get("actual"):
                item["actual"] = abs(float(actuals["special_projects_total"]))
                item["desc"] = "Special Projects (from Actuals)"
                break
...
def main():
    ...
    if logo_path.exists():
        ...
        with col_title:
            ...
            st.caption("Upload WeConnectU Actual vs Budget Excel → extract actuals → review → download Excel")
    else:
        st.title("Domus Property Management Budget")
        st.caption("Upload WeConnectU Actual vs Budget Excel → extract actuals → review → download Excel")
    ...
    with st.sidebar:
        ...
        st.header("2. Upload Actuals")
        st.caption("Preferred: WeConnectU **Actual vs Budget** Excel export")
        uploaded_excel = st.file_uploader(
            "Actual vs Budget (Excel)",
            type=["xlsx", "xls"],
            key="excel_actuals",
            help="Export from WeConnectU → Actual vs Budget report",
        )

        if uploaded_excel and st.button("Extract Actuals from Excel", type="primary"):
            with st.spinner("Reading Excel…"):
                try:
                    actuals, detected_name, period = extract_actuals_from_excel(uploaded_excel)
                    st.session_state.actuals = {k: v for k, v in actuals.items() if not str(k).startswith("_")}
                    st.session_state.unmatched_lines = actuals.get("_unmatched", [])
                    st.session_state.sections = apply_actuals(default_sections(), st.session_state.actuals)

                    if detected_name and not st.session_state.complex_name:
                        st.session_state.complex_name = detected_name
                    if period:
                        st.session_state.fin_year = period

                    rec = (
                        abs(float(actuals.get("ins_claims_received") or 0))
                        + abs(float(actuals.get("ins_claims_rm") or 0))
                        + abs(float(actuals.get("ins_premium_recovered") or 0))
                    )
                    if rec:
                        st.session_state.insurance_recoveries = rec

                    for k in list(st.session_state.keys()):
                        if k.startswith("df_"):
                            del st.session_state[k]

                    n_mapped = len([k for k in st.session_state.actuals if st.session_state.actuals[k]])
                    st.success(f"Extracted {n_mapped} line items from Excel. Review the tabs.")
                    if st.session_state.unmatched_lines:
                        with st.expander(f"{len(st.session_state.unmatched_lines)} unmatched lines (review)"):
                            for lab, amt in st.session_state.unmatched_lines[:40]:
                                st.write(f"• {lab}: R {amt:,.2f}")
                    st.rerun()
                except Exception as e:
                    st.error(f"Could not read Excel: {e}")

        with st.expander("Or upload PDF (fallback)"):
            uploaded_pdf = st.file_uploader("Financial statement (PDF)", type=["pdf"], key="pdf_actuals")
            if uploaded_pdf and st.button("Extract Actuals from PDF"):
                with st.spinner("Reading PDF…"):
                    ...
                    for k in list(st.session_state.keys()):
                        if k.startswith("df_"):
                            del st.session_state[k]
                    st.success("PDF extraction complete. Review the numbers in the tabs.")
                    st.rerun()
